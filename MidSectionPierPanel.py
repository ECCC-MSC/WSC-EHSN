...
class PierPanel(wx.Panel):
    def __init__(self, modify, pid, nextTagMark="", *args, **kwargs):
        super(PierPanel, self).__init__(*args, **kwargs)
        self.nextTagMark = nextTagMark
        self.modify = modify
        self.pid = pid
        self.titleLbl = "Pier / Island"
        self.panelLbl = "Panel #:"
    ...
    def OnSave(self, event):
        ...
        # if self.startEdgeRdBtn.GetValue():
        #     if not failed==-1:
        #         table=self.GetParent().GetParent().GetParent()
        #         if table.modifiedPanelArrayIndex == len(table.panelObjs) - 1\
        #              or table.panelObjs[table.modifiedPanelArrayIndex+1].panelNum != "End Pier":
        #             table.Adding(endPier=True)
        event.Skip()
    ...
    def SaveToObj(self):
        ...
        if table.IsValidTagMark(dist, pid=self.pid, arrayIndex=arrayIndex, modify=self.modify, startPier=startOrEndVal, endPier=(not startOrEndVal)) != 0:
            return -1
        ...
        obj = EdgeObj(edgeType="Pier", time=measurementTime, distance=dist, startOrEnd=startOrEndVal, panelNum=panelNum,\
            depth=depthEdge, corrMeanVelocity=velEdge, depthAdjacent=depthAdjacentCkbox, velocityAdjacent=velocityAdjacentCkbox, pid=self.pid)
    ...
    def OnSaveAddNext(self, event):
        ...
        if failed != -1:
            startPier = self.startEdgeRdBtn.GetValue()
            if startPier:

                self.GetParent().GetParent().GetParent().Adding(endPier=startPier, insert=self.modify)
    ...
    def OnCancel(self, event):
        ...
        if self.modify != 2:
            ...
            if self.endEdgeRdBtn.GetValue():
                ...
                dlg = wx.MessageDialog(self, "Do you want to cancel adding piers?", 'None', wx.YES_NO)
                res = dlg.ShowModal()
                if res == wx.ID_NO:
                    dlg.Destroy()
                    return
            ...
            startTableIndex = table.GetTableIndex(table.panelObjs[startArrayIndex])
            # print "startArrayIndex----", startArrayIndex
            # print "startTableIndex----", startTableIndex
            if startArrayIndex is not None and startTableIndex != -1:
                table.Remove(startArrayIndex, startTableIndex)
