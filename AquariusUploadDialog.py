...
class AquariusUploadDialog(wx.Dialog):
    ...
    def OnChange(self, e):
        if self.GetParent().name == '':
            date = datetime.datetime.strptime(str(self.GetParent().manager.genInfoManager.datePicker),
                                              self.GetParent().manager.DT_FORMAT)
            date = date.strftime("%Y%m%d")
            name = str(self.GetParent().manager.genInfoManager.stnNumCmbo) + "_" + str(date) + "_FV.pdf"

        else:
            name = self.GetParent().name.replace("xml", "pdf")
        fileOpenDialog = wx.FileDialog(self, self.changeTitleLbl, self.GetParent().uploadSaveDir, name,
...
# Implementing Zip
    def UploadAQ(self,evt):
        ...
        if os.path.exists(self.GetParent().attachment.zipPath):
                ...
                try:
                    ...
                    if res == wx.ID_YES:
                        ...
                        if self.GetParent().manager is not None:
                            ...
                            result = self.GetParent().manager.ExportToAquarius(server, username, password, fvDate,
                                                                               withDischarge, withLevelNote)
                            show_error = False
                            uploadInfo = []
                            if result is not None:
                                if "password" in result:
                                    ...
                                    self.GetParent().manager.UploadInfoAsXMLTree(
                                        self.GetParent().manager.uploadRecord,
                                        uploadInfo)
                            ...
                            #######################################################
                                ...
                                self.GetParent().ResetUploadSaveIni()
                                ...
                                if self.GetParent().manager.uploadRecord is None:
                                    self.GetParent().manager.uploadRecord = Element('AQ_Upload_Record')
                                self.GetParent().manager.UploadInfoAsXMLTree(self.GetParent().manager.uploadRecord,
                                                                             uploadInfo)
                ...
                try:
                    ...
                    if res == wx.ID_YES:
                        if self.GetParent().manager is not None:
                            ...
                            print fvPath
                            result = self.GetParent().manager.ExportToAquariusNg(server, username, password, fvPath,
                                                                                 fvDate)
                            if result != None:
                                error = wx.MessageDialog(None, result, 'Error',
                                                         wx.OK | wx.ICON_ERROR)
